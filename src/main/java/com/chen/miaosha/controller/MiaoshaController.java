...
import com.chen.miaosha.rabbitmq.MQSender;
...
public class MiaoshaController implements InitializingBean {
    ...
    public Result<Integer> miaosha(Model model,MiaoshaUser user,
    	...
    	if(stock < 0) {
			...
			redisService.incr(GoodsKey.getMiaoshaGoodsStock, ""+goodsId);//+1  //为了当消息队列消费时下单不成功能够保持数据一致
    		return Result.error(CodeMsg.MIAO_SHA_OVER);
    	}
    	//判断是否已经秒杀到了
    	...
    	if(order != null) {
			redisService.incr(GoodsKey.getMiaoshaGoodsStock, ""+goodsId);//+1  //为了当消息队列消费时下单不成功能够保持数据一致
			localOverMap.put(goodsId, false);//放开
    ...
    public Result<String> getMiaoshaPath(HttpServletRequest request, MiaoshaUser user,
    	...
    	boolean check = miaoshaService.checkVerifyCode(user, goodsId, verifyCode);//验证数学公式验证码
    	if(!check) {
    		return Result.error(CodeMsg.REQUEST_ILLEGAL);
    	}
